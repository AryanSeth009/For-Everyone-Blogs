...
async function fixAllBlogsAuthors() {
  try {
    // Get all blogs
...
server.post("/create-blog", verifyJWT, async (req, res) => {
  try {
    ...
    console.log("Creating blog with author:", {
      userId: user_id,
      username: user.personal_info.useGGrname,
    });
...
server.post("/fix-blog-relationships", async (req, res) => {
  try {
    // List of blog IDs and author usernames
    const blogAuthors = [
      {
        blog_id: "geeksforgeeks-1",
        author_username: "geeksforgeeks",
      },
      {
        blog_id: "freecodecamp-1",
        author_username: "freecodecamp",
      },
      {
        blog_id: "socialmediaexaminer-1",
        author_username: "socialmediaexaminer",
      },
      {
        blog_id: "nomadicmatt-1",
        author_username: "nomadicmatt",
      },
    ];

    // Update blog relationships
    await Promise.all(
      blogAuthors.map(async (blogAuthor) => {
        // Find the user
        const user = await User.findOne({
          "personal_info.username": blogAuthor.author_username,
        });
        ...
        // Find the blog
        const blog = await Blog.findOne({ blog_id: blogAuthor.blog_id });
        if (!blog) {
          return res.status(404).json({ error: "Blog not found" });
        }
